...
/*   Updated: 2022/04/22 03:57:21 by nwattana         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	sub_put(char *str, t_format *f1);
...
int	put_x_p(unsigned long long number, t_format *f1)
{
	...
	number_len = number_base_len_p(number, 16, f1);
	check_format_x(f1, number_len);
	num_a = ft_itoa_hexa_p(number, number_len, f1);
	res = 0;
	if (f1->left)
	{
		res += sub_put(num_a, f1);
...
static int	check_format_x(t_format	*f1, int number_len)
{
	...
	if (f1->dot)
		width = number_len + 2 * f1->hash - f1->zero_value;
	else
		width = number_len + 2 * f1->hash;
...
static int	sub_put(char *num_a, t_format *f1)
{
	int	res;

	res = 0;
	res += put_charloop('0', 1);
	res += put_charloop('x', 1);
